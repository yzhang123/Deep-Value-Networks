...
import sys
import logging
from dvn.src.util.measures import _oracle_score
...
class DvnNet(object):
    ...
    def _create_loss(self, score, y, y_gt):
        ...
        :return:
        """
        sim_score = _oracle_score(y + 0.5 * tf.ones_like(y), y_gt + 0.5 * tf.ones_like(y)) # shape=[batchsize x 1]
        ...
        tf.summary.scalar('mean_simscore-netoutput', tf.reduce_mean(tf.abs(self._graph['score_diff'])))
    ...
    def build_network(self, train=True):
        ...
        with tf.variable_scope('input'):
            ...
            self._graph['x'] = tf.placeholder(tf.float32, shape=[self._batch_size, None, None, 3], name='x-input') # 3+k between 0 and 1
            tf.summary.image("x", self._graph['x'])
            self._graph['y_gt'] = tf.placeholder(tf.float32, shape=[self._batch_size, None, None, self._num_classes], name='y-gt') # ground truth segmentation
            self._graph['y'] = tf.placeholder(tf.float32, shape=[self._batch_size, 24, 24, self._num_classes], name='y')
        ...
        #     self._graph['conv1'] = self.conv_acti_layer(self._graph['y'], [5,5], 64, "conv1", 1, layernorm=True)
        ...
        #     self._graph['conv4'] = self.conv_acti_layer(self._graph['conv3'], [5, 5], 4, "conv4", 1, layernorm=True)
        #
        # #conv3_flat = tf.reshape(self._graph['conv3'], [-1, 6*6*128], name='pre_fc')
        # conv3_flat = tf.reshape(self._graph['conv4'], [-1, 6*6*4], name='pre_fc')
        # # self._graph['fc1'] = self.fc_acti_layer(conv3_flat, weight_shape=[6 * 6 * 128, 384], bias_shape=[384], name='fc1', dropout=False, layernorm=True)
        ...
        # self._graph['fc3'] = self.fc_acti_layer(conv3_flat, weight_shape=[6 * 6*4, 1], bias_shape=[1], activation_fn=tf.nn.sigmoid, name='fc3')
        #
        self._graph['fc1'] = fully_connected(self._graph['y'],20, 'fc1')
        self._graph['fc2'] = fully_connected(self._graph['fc1'], 1, 'fc2', activation_fn=tf.nn.relu)
        self._graph['fc3'] = tf.nn.sigmoid(self._graph['fc2'])
