...
class DvnNet(object):
    ...
    def conv_acti_layer(self, bottom, filter_shape, filter_depth, name, stride, padding='SAME'):
        ...
        with tf.variable_scope(name):
            ...
            weight = self._weight_variable(weights_shape, initializer=tf.orthogonal_initializer(gain=1.0, seed=None), name=name)
            bias = self._bias_variable(filter_depth, initializer=tf.constant_initializer(0.001), name=name)
    ...
    def build_network(self, train=True):
        ...
        with tf.variable_scope('inference'):
            ...
            self._graph['inference_update'] = self._graph['y1'].assign(
                tf.clip_by_value(tf.subtract(self._graph['identity'], self._graph['inference_grad'][0]), 0., 1.))
